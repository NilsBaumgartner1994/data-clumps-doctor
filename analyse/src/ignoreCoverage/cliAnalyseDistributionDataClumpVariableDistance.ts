...
import { DataClumpsTypeContext, DataClumpsVariableFromContext, DataClumpsVariableToContext, DataClumpTypeContext, Dictionary, Position } from 'data-clumps-type-context';
 ...
 */
function adjustPositionForModifiersTypeAndDelimiters(position: Position, modifiers: string[] | undefined, fullyQualifiedType: string, variableName: string, adjustForModifiersAndTypes: boolean, isFirst: boolean): Position {
 ...
 */
function adjustPositionsForModifiersAndType(variables: DataClumpsVariableFromContext[] | DataClumpsVariableToContext[], adjustForModifiersAndType: boolean): Position[] {
  ...
  return variables.map(variable => {
    ...
    index++;
    return adjustPositionForModifiersTypeAndDelimiters(variable.position, variable.modifiers, variable.type, variable.name, adjustForModifiersAndType, isFirst);
  });
}
 ...
 * @throws {Error} Throws an error if the input variables are not of the expected types.
 */
function calculateFieldDistances(variables: DataClumpsVariableFromContext[] | DataClumpsVariableToContext[]): { pairwise: NumberOccurenceDict; spread: NumberOccurenceDict; toMedian: NumberOccurenceDict } {
  ...
  for (let i = 0; i < total_amount_of_report_files; i++) {
    ...
    for (let j = 0; j < amount_of_data_clumps; j++) {
      ...
      if (dict_of_analysed_data_clumps_keys[data_clump_key] === true) {
        // Skip already analysed data clumps
        continue;
      } else {

...
export class AnalyseHelper {
  ...
  static getCommandForAnalysis(
    argv: NodeJS.Process | string[],
    options: {
      description?: string;
    ...
    program.description(options?.description || 'Analyse Detected Data-Clumps\n\n' + 'This script performs data clumps detection in a given directory.\n\n' + 'npx data-clumps-doctor [options] <path_to_folder>').version(version);
    ...
    program.option('--output <path>', 'Output path for script', current_working_directory + '/' + (options.default_output_filename_without_extension || 'data-clumps-doctor-output-' + Analyzer.project_name_variable_placeholder) + '.py'); // Default value is './data-clumps.json'
...
` + '\n'
  ...
  static getPythonPlot(options: { output_filename_without_extension?: string; y_label: string; y_max?: number; y_ticks?: number; offset_left: number; offset_right: number; offset_top: number; offset_bottom: number; width_inches: number; height_inches: number; w_bar_width?: number; x_labels?: string[]; use_manual_labels?: boolean; horizontal?: boolean; log_scale?: boolean; log_ticks?: number[]; y_lines?: number[] }) {

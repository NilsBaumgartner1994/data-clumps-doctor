...
program
  ...
  .option(
    '--git_project_urls_to_analyse <git_project_urls_to_analyse>',
    'Git project URLs to analyse. Comma separated list of URLs.',
    val => {
  ...
  .option(
    '--detector_options_ignored_variable_names <names>',
    'List of ignored variable names (supports [a,b,c])',
    val => {
      return parseArrayFromString(val);
    },
    []
...
async function analyse(path_to_project, options) {
  ...
  let analyzer = new Analyzer(path_to_project, path_to_ast_generator_folder, path_to_output_with_variables, absolute_path_to_source, source_type, path_to_ast_output, commit_selection_mode, passed_project_url, git_tag_start_offset, passed_project_name, project_version, preserve_ast_output, detector_options);

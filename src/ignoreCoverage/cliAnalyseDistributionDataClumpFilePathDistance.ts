...
import { DataClumpsTypeContext, DataClumpsVariableFromContext, DataClumpTypeContext, Dictionary } from 'data-clumps-type-context';
...
function getFilePathDistance(filePathA: string, filePathB: string) {
  ...
  let common = 0;
  while (common < aFolders.length && common < bFolders.length && aFolders[common] === bFolders[common]) {
    common++;
  }

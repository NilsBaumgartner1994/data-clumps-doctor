...
async function analyseFileStream(report_file_path: string, data_clump_from_file_distribution: Record<string, number>, dict_of_analysed_data_clumps_keys: Record<string, boolean>) {
  return new Promise<void>((resolve, reject) => {
    const pipeline = chain([fs.createReadStream(report_file_path), parser(), pick({ filter: 'data_clumps' }), streamValues()]);
...
async function analyse(report_folder, options) {
  ...
  const data_clump_from_file_distribution_sorted = Object.keys(data_clump_from_file_distribution).sort((a, b) => data_clump_from_file_distribution[b] - data_clump_from_file_distribution[a]);
